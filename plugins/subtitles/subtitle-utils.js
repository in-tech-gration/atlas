...
export function youTubeTranscript2SRT(transcript) {
  ...
  transcript.forEach((t, index, array) => {
    ...
    const text = t.t._;
    const startTime = Number(t.t['$'].start).toFixed();
    const duration = Number(t.t.$.dur).toFixed();
    ...
    const nextStartTime = isNotLastElement ? Number(array[index + 1].t['$'].start).toFixed() : Infinity;
