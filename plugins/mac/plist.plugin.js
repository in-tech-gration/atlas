...
// Based on: https://github.com/joeferner/node-bplist-parser
...
const parseFile = function (fileNameOrBuffer, callback = () => { }) {
...
const parseFileSync = function (fileNameOrBuffer) {
  if (!Buffer.isBuffer(fileNameOrBuffer)) {
...
/**
 * ⚠️ WORK IN PROGRESS
 * @description Read binary plist files on MacOS
 */
export default async function plist(options, globalOptions, cliInstance) {

  try {

    const plistFile = options[0];
    const obj = await parseFile(plistFile);
